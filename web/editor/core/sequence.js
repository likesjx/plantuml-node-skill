...
export function parseSequencePuml(source) {
...
export function stringifySequenceModel(model) {
  const out = [START];

  for (const participant of model.participants) {
    out.push(`participant ${participant.name}`);
  }

  out.push("");

  for (const message of model.messages) {
    const from = model.participants.find((p) => p.id === message.from);
    const to = model.participants.find((p) => p.id === message.to);
    if (!from || !to) {
      continue;
    }
    const arrow = message.arrow === "-->" ? "-->" : "->";
    out.push(`${from.name} ${arrow} ${to.name}: ${message.text || "message"}`);
  }
...
export function createDefaultModel() {
  return {
    ...
    messages: [{ id: "m_1", from: "alice", to: "bob", arrow: "->", text: "Hello" }],
  };
}
